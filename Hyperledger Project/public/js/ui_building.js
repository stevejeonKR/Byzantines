...
function build_marble(marble) {
	...
	html += '<span id="' + marble.id + '" class="ball ' + size + ' ' + colorClass2 + ' ' + auditing + ' title="' + marble.id + '"';
...
function build_company_panel(company) {
	...
	var html = `<div class="companyPanel" company="` + company + `">
					...
					<span class="companyName">` + namecom + `&nbsp;-&nbsp;</span>
...
function build_a_tx(data, pos) {
	...
	html += `<div class="txDetails">
				<div class="txCount">거래번호: ` + (Number(pos) + 1) + `</div>
				<p>
					<div class="marbleLegend">트랜잭션: </div>
					<div class="marbleName txId">` + data.txId.substring(0, 14) + `...</div>
				</p>
				<p>
					<div class="marbleLegend">소유자: </div>
					<div class="marbleName">` + username + `</div>
				</p>
				<p>
					<div class="marbleLegend">보유회사: </div>
					<div class="marbleName">` + unn + `</div>
				</p>
				<p>
					<div class="marbleLegend">소유자 ID: </div>
